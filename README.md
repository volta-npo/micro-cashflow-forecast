...
# 💸 Micro Cashflow Forecast

### A tiny, owner-friendly cashflow model for businesses that do not need a full finance stack.
...
**Finance & Grants** · **Repo 18/50** · **No backend. No login. Client data stays local.**

[Live app](https://volta-npo.github.io/18-micro-cashflow-forecast/) · [Report an issue](https://github.com/volta-npo/18-micro-cashflow-forecast/issues) · [Volta](https://voltanpo.org)
...
**Micro Cashflow Forecast** is a polished, local-first open-source tool from Volta's 50-project OSS division. It helps Finance students helping owners understand next month, not Wall Street turn real community work into structured evidence, client-safe handoffs, and mentor-reviewable release packets.
...
Existing finance tools are too complex or too accounting-specific for first advisory conversations.

### The niche

Restaurants, salons, and local service businesses.

### North-star metric

`owners who understand cash runway scenarios`
...
flowchart TD
    ...
    Evidence --> Workbench["Micro Cashflow Forecast"]
    Workbench --> Score["Funding readiness"]
    Score --> Cert["v3 production certification"]
    Cert --> A1["13-week forecast CSV"]
    Cert --> A2["Owner-friendly summary"]
    Cert --> A3["Scenario comparison"]
...
flowchart LR
    subgraph Modules["First-class modules"]
        M1["Weekly cash model"]
        M2["Scenario toggles"]
        M3["Owner explanation notes"]
        M4["Risk flags"]
    end
    M1 --> Workbench
    M2 --> Workbench
...
sequenceDiagram
    ...
    participant T as Micro Cashflow Forecast
...
|---|---|
| **Domain workbench** | A purpose-built cashflow interface for a tiny, owner-friendly cashflow model for businesses that do not need a full finance stack. |
...
|---:|---|---|
| 1 | **Weekly cash model** | Converts field work into repeatable, reviewable Volta delivery evidence. |
| 2 | **Scenario toggles** | Converts field work into repeatable, reviewable Volta delivery evidence. |
| 3 | **Owner explanation notes** | Converts field work into repeatable, reviewable Volta delivery evidence. |
| 4 | **Risk flags** | Converts field work into repeatable, reviewable Volta delivery evidence. |
...
```bash
git clone https://github.com/volta-npo/18-micro-cashflow-forecast.git
cd 18-micro-cashflow-forecast
npm test
npm start
```
